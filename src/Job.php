...
class Job {
    public static function create($options=array()) {
        ...
        return Videoapicloud::submit(Videoapicloud::config($options), $api_key);
    }

    public static function get($jid, $options=array()) {
        $api_key = self::getApiKey($options);

        return Videoapicloud::get('/v1/jobs/' . $jid, $api_key);
    }

    public static function getAllMetadata($jid, $options=array()) {
        $api_key = self::getApiKey($options);

        return Videoapicloud::get('/v1/metadata/jobs/' . $jid, $api_key);
    }

    public static function getMetadataFor($jid, $source_or_output, $options=array()) {
        $api_key = self::getApiKey($options);

        return Videoapicloud::get('/v1/metadata/jobs/' . $jid . '/' . $source_or_output, $api_key);
    }
